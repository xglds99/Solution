...
public class l1482 {
    class Solution {
        public int minDays(int[] bloomDay, int m, int k) {
            ...
            if(cnt > n)return -1;
            int l = 1;
            int maxValue = Arrays.stream(bloomDay)
                    .max()
                    .getAsInt();
            ...
            return Math.min(l, maxValue);
        }


        public boolean  f(int []bloomDay, int day, int m,int k){
            long cnt = 0L;
            long cur = 0L;
            for(int b : bloomDay){
                if(b <= day)cur ++;
